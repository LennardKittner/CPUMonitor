...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        try! FileManager.default.createDirectory(atPath: (URL(fileURLWithPath: "\(FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0].path)/CPUMonitor").path), withIntermediateDirectories: true, attributes: nil)

        if !FileManager.default.fileExists(atPath: (conf_file.path)) {
            FileManager.default.createFile(atPath: (conf_file.path), contents: ("memdetail:\((startAtLogin ? "yes" : "no"))\nAtLogin:\((startAtLogin ? "yes" : "no"))").data(using: String.Encoding.utf8, allowLossyConversion: false)!, attributes: nil)
        }
        else {
        ...
        oldUsage = (cpuUsage?.user)!+(cpuUsage?.system)!
    ...
    func refreshIcon(usage: Double){
        ...
        statusItem.button?.image = icons[index]
    }
    ...
    func refreshMenu() {
        ...
        let ram = NSMenuItem(title: "Memory: \(String(format: "%.2f", memoryd)) GB / \(memory_max) GB ", action: nil, keyEquivalent: "")
